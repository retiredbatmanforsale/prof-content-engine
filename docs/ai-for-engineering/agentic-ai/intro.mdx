...
- Basic familiarity with `async`/`await` (lesson 5 introduces it from zero, but a passing acquaintance helps)
- **No prior LLM or API key required** — the course uses a `MockLLM` throughout; you can swap to a real LLM at the end
